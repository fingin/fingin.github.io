    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=1156&wager=9900000&limit=11&order_by=desc", function(json){
...
$('#depositButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/deposit?app_id=1156';
...
$('#withdrawButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/withdraw?app_id=1156';
    var windowName = 'manage-auth';
    var windowOpts = 'width=420,height=350,left=100,top=100';
    var windowRef = window.open(windowUrl, windowName, windowOpts);
    windowRef.focus();
...
function onloadCallback() {
    grecaptcha.render('faucetClaimCaptcha', {
        'sitekey' : '6LcEkiATAAAAAPaORxYtiJqaiJvsujUT4YVf1zjI',
...
function startNewGame(){
    if(!connected){
        $('#theDimmer').html("Error: Not connected!<br><a href='https://www.moneypot.com/oauth/authorize?app_id=1156&response_type=token&state=Meh&redirect_uri=http://fingin.github.io/'>Click here to login</a>");
    ...
    $.post("https://api.moneypot.com/v1/hashes?access_token="+getURLParameter('access_token'), '', function(json) {
        ...
        game.tilesClicked = 0;
        houseEdge = 0.001; // 1%
        odds = ( (25-game.bombs-game.tilesClicked-8) / (25-game.tilesClicked) );
